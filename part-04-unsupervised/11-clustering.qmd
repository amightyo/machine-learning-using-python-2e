...
- evaluate clustering of visual data as a representation-dependent exploratory task.
...
## Are Clusters Discovered or Constructed?
...
### The Same Observations, Different Representations

Consider observations measured on two variables with very different scales.

```{python}
#| label: fig-clustering-representation
#| fig-cap: "Clustering can change when feature scaling changes the geometry used by the algorithm."
#| fig-alt: "Scatterplot showing observations grouped into distinct regions, illustrating how clustering identifies structure in unlabeled data."
#| warning: false
...
### Clusters Are Analytical Claims
...
- internal validity;
- external or substantive validity;
- domain interpretation.

::: {.callout-note icon=false}
## Beyond the Algorithm: Categories Can Become Real Through Use

Once analysts name clusters, organizations may begin treating them as meaningful categories.

Labels such as “high-risk learner,” “complex patient,” or “premium customer” can influence interventions, resources, and expectations.

This creates an important responsibility: a computational grouping can become socially consequential even when its boundaries were analytically constructed.
:::

::: {.callout-important icon=false}
## Research Reality Check

Finding \(k=3\) clusters does not demonstrate that three natural populations exist.

A defensible claim is narrower:

> Under this representation, distance measure, algorithm, and validation procedure, a three-cluster solution produced useful structure.

That is a stronger scientific statement because it makes the assumptions visible.
:::

## $k$-Means Clustering

$k$-means seeks $K$ cluster centers, or centroids, following one of the foundational formulations of partition-based clustering [@macqueen1967]:
...
#| fig-alt: "Elbow plot of within-cluster variation across values of k, with diminishing improvement as additional clusters are added."
...
Silhouette analysis provides an internal diagnostic of how well an observation fits its assigned cluster relative to neighboring clusters [@rousseeuw1987].
...
#| fig-alt: "Silhouette scores across candidate numbers of clusters, used to compare cluster separation and cohesion."
...
#| fig-alt: "Hierarchical clustering dendrogram showing successive merges of observations and clusters at increasing linkage distances."
...
#| fig-alt: "Two interleaving crescent-shaped groups of observations illustrating a nonlinear cluster structure."
...
#| fig-alt: "K-means assignments on the two-moons data, showing partitions that do not fully capture the curved cluster structure."
...
#| fig-alt: "Clustering result illustrating how an unsuitable number of clusters can force artificial groupings in the data."
...
## Computer Vision Learning Experience: Clustering Images Without Labels
...
adjusted_rand_score(y_cluster_cv, clusters_digits_cv)
```

The adjusted Rand index compares the discovered partition with known labels while accounting for agreement expected by chance. Cluster numbers do not need to match digit numbers.
...
If image clusters resemble known classes, that is evidence that the chosen representation contains class-related structure. It does not establish that the clusters are natural visual kinds or that the same structure will appear under another representation.
:::
...
21. Why can feature scaling change a clustering solution?
22. In what sense can clusters be partly constructed rather than merely discovered?
23. What evidence would strengthen a claim that a cluster solution is substantively meaningful?
...
- Clusters depend on representation, distance, algorithm, and tuning choices.
- Cluster labels should not automatically be treated as natural categories.
- Stability and substantive validity matter alongside internal clustering metrics.
