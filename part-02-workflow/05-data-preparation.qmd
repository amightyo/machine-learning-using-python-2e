...
A data-centric perspective asks whether better evidence could matter more than greater model complexity. Research on data cascades in high-stakes AI similarly shows how upstream data problems can propagate through downstream systems [@sambasivan2021data]. Potential improvements include better labels, measurement, population coverage, temporal alignment, category consistency, feature representation, and removal of corrupted or duplicated observations.
...
#| fig-alt: "Bar chart comparing test accuracy for logistic regression trained on noisy labels, random forest trained on the same noisy labels, and logistic regression trained on clean labels. The clean-label model performs best, emphasizing that better training evidence can matter more than greater model complexity."
#| warning: false
...
Missing values may arise because a variable was not measured, not applicable, refused, structurally unavailable, lost, or recorded under a special code. The missingness process matters for valid inference; missing values should therefore not be treated as a purely mechanical cleaning problem [@rubin1976missing].
...
## Computer Vision Data: Representation Is Preprocessing

Images require the same disciplined thinking as tabular data, but their structure introduces additional choices.

An image may be represented as

$$
I_i\in\mathbb{R}^{H\times W}
$$

for grayscale data or
...
Common preparation decisions include:

- resizing and cropping;
- pixel-value scaling or normalization;
- color-space or channel handling;
- augmentation;
- image-quality screening;
- class-label verification;
- duplicate and near-duplicate detection;
- grouping related images from the same person, object, session, or source.

### Flattening Changes the Representation

Classical algorithms often require a feature vector:
...
For an \(8\times8\) grayscale image, flattening produces \(p=64\) pixel features.

```{python}
from sklearn.datasets import load_digits

digits_cv = load_digits()

X_images_cv = digits_cv.images
X_flat_cv = digits_cv.data
y_cv = digits_cv.target

X_images_cv.shape, X_flat_cv.shape, y_cv.shape
```
...
### Augmentation Belongs Inside the Training Process
...
::: {.callout-note icon=false}
## A Vision Transformation Is Also an Assumption
...
This becomes especially important during cross-validation because each training fold should learn its own preprocessing parameters. scikit-learn's estimator and pipeline ecosystem supports composable preprocessing and modeling workflows [@pedregosa2011scikit].
...
#| fig-alt: "Two-bar comparison of logistic-regression test accuracy using legitimate features versus features that include the target itself. Accuracy becomes nearly perfect when the target leaks into the predictors, illustrating misleading performance caused by leakage."
...
1. inspect the original image tensor and flattened feature matrix;
2. visualize examples from several classes;
3. document the unit of analysis and target;
4. identify an appropriate train/test split;
5. scale pixel features inside a leakage-safe pipeline;
6. fit a simple baseline classifier;
7. evaluate performance on unseen images;
8. inspect several misclassified images;
9. explain what information is lost or obscured when images are flattened;
10. describe how the evaluation design would need to change if multiple images came from the same person or acquisition session.
