...
### From Building Systems to Evaluating Evidence
...
ROC analysis summarizes discrimination across thresholds by tracing the tradeoff between true-positive and false-positive rates [@fawcett2006roc].
...
#| fig-alt: "ROC curve plotting true positive rate against false positive rate across classification thresholds, with a diagonal chance-reference line and the model's ROC-AUC shown in the legend."
...
When positive cases are uncommon, precision-recall analysis can be especially informative; precision-recall curves can reveal differences that ROC curves may obscure under substantial class imbalance [@saitoRehmsmeier2015].

```{python}
#| label: fig-pr
#| fig-cap: "Precision-recall curves emphasize performance on the positive class."
#| fig-alt: "Precision-recall curve plotting precision against recall across classification thresholds, with the model's average precision summarized in the legend."
...
Probability calibration should be evaluated separately from discrimination when predicted probabilities will support decisions [@niculescuMizilCaruana2005]. Modern neural networks can be highly accurate while remaining poorly calibrated [@guoEtAl2017].
...
The Brier score is a classic proper scoring rule for probabilistic forecasts [@brier1950].
...
#| fig-alt: "Calibration curve comparing mean predicted probabilities with observed positive fractions across probability bins, alongside a diagonal line representing perfect calibration."
...
#| fig-alt: "Line chart showing sensitivity and specificity across decision thresholds, illustrating how increasing or decreasing the threshold changes the balance between the two metrics."
...
Nested cross-validation separates model selection from model assessment, reducing the optimistic bias that arises when the same resampling process is used both to tune and to report performance [@varma2006bias].
...
#| fig-alt: "Histogram of bootstrap ROC-AUC estimates for the fixed fitted model, with dashed vertical lines marking the lower and upper confidence interval bounds."
...
When many algorithms are compared across datasets or repeated resampling results, apparent rank differences should not be overinterpreted without appropriate paired statistical comparison [@demsar2006].
...
This can reveal failures invisible to internal resampling and is central to judging whether predictive performance transports beyond the development setting [@steyerberg2010assessing].
