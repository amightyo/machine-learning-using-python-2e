...
The modern classification-and-regression-tree framework is classically associated with CART [@breimanEtAl1984cart].
...
#| fig-alt: "Diagram of a shallow classification tree. Internal nodes show a hierarchy of feature-threshold splits, branches show the resulting decisions, and terminal nodes summarize the predicted class composition."
...
#| fig-alt: "Training and test accuracy plotted across increasing maximum tree depth. Training accuracy rises as depth increases, while test accuracy improves only to a point and then levels off or deteriorates, illustrating overfitting."
...
Bagging reduces variance by aggregating predictors fitted to bootstrap samples, an approach introduced by Breiman [@breiman1996bagging].
...
#| fig-alt: "Horizontal bar chart showing how often each feature is selected as the root split across 250 bootstrap samples. A dominant bar indicates a stable first split, while several similarly sized bars would indicate root-feature instability."
#| warning: false
...
Random Forests extend bagging by adding **feature randomness**, reducing correlation among trees while retaining the variance-reduction benefits of aggregation [@breiman2001randomForests].
...
#| fig-alt: "Grouped horizontal bar chart comparing impurity-based importance with permutation importance for leading features. Differences between paired bars show that the two importance methods can rank model dependence differently."
...
#| fig-alt: "Eight-by-eight heatmap of Random-Forest impurity importance reshaped to the original handwritten-digit image geometry. Stronger cells indicate pixel locations on which the fitted forest depends more heavily for classification."
#| warning: false
