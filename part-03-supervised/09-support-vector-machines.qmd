...
Support-vector machines formalize classification around maximum-margin separation, with the canonical support-vector network formulation developed by Cortes and Vapnik [@cortesVapnik1995].
...
#| fig-alt: "Two-class scatterplot with a linear SVM separating hyperplane, two parallel margin boundaries, and support vectors outlined with large circles. The support vectors lie closest to the margin and determine the fitted maximum-margin boundary."
...
#| fig-alt: "Piecewise hinge-loss curve plotted against signed margin. Loss decreases linearly for margins below one and becomes zero once the signed margin reaches or exceeds one, marked by a vertical reference line."
...
#| fig-alt: "Scatterplot of two classes arranged as noisy concentric circles. Because one class surrounds the other, no single straight line can separate the classes in the original two-dimensional representation."
...
This is the **kernel trick**, a central idea in kernel methods that allows algorithms to operate through inner products in an implicit feature space [@scholkopfSmola2002].
...
#| fig-alt: "Two related scatterplots showing a nonlinear feature transformation. In the original one-dimensional representation, class membership changes at both ends of the axis; after adding the squared feature, the classes become geometrically easier to separate."
#| warning: false
...
#| fig-alt: "Comparison of decision boundaries for linear and radial-basis-function SVMs on concentric-circle data. The linear model cannot follow the circular class structure, whereas the RBF model forms a curved boundary that separates the inner and outer classes."
...
#| fig-alt: "Four RBF-SVM decision-boundary plots using different combinations of C and gamma. Lower values produce smoother, less flexible boundaries, while larger values can produce tighter and more locally responsive boundaries around the training observations."
